...
export default function SuspendedUsersPage() {
  ...
  const handleDelete = async (userId: string) => {
    try {
      setActionLoading(userId);
      await userService.suspend(userId, "Permanently suspended by admin");
      refetch();
    } catch (error) {
  ...
  if (error) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>
          Failed to load suspended users. Please try again.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Suspended Users</h1>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            A list of all suspended user accounts
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
